select p.d_descred cod_programa,
      ...
      --- ALE 05/03/21
       (select  --ba.D_BECAS
                LISTAGG(ba.D_BECAS, '; ') WITHIN GROUP (ORDER BY ba.f_desde)
            from VW_BECAS_CONCATENADAS ba
        where 1=1
        and ba.n_id_alu_prog = ap.n_id_alu_prog
        --and fun_between_fechas(ba.f_desde, ba.f_hasta,:p1_fecha_desde,:p1_fecha_hasta) = 'S'
        --and fun_between_fechas(ba.f_desde, ba.f_hasta,to_date('01/03/'||to_char(to_date(:p1_fecha_hasta,'dd/mm/yyyy'),'yyyy'),'dd/mm/yyyy'),:p1_fecha_hasta) = 'S'
        and :p1_fecha_hasta between ba.f_desde and ba.f_hasta
       ) becas,
       (select  --ba.f_desde
                LISTAGG(ba.f_desde, '; ') WITHIN GROUP (ORDER BY ba.f_desde)
          from VW_BECAS_CONCATENADAS ba
        where 1=1
        and ba.n_id_alu_prog = ap.n_id_alu_prog
        --and fun_between_fechas(ba.f_desde, ba.f_hasta,:P1_f_desde,:P1_F_hasta) = 'S'
        --and fun_between_fechas(ba.f_desde, ba.f_hasta,to_date('01/03/'||to_char(to_date(:p1_fecha_hasta,'dd/mm/yyyy'),'yyyy'),'dd/mm/yyyy'),:p1_fecha_hasta) = 'S'
        and :p1_fecha_hasta between ba.f_desde and ba.f_hasta
       ) desde,
        (select  --ba.f_hasta
...
--- Ticket #014749, Tomás Herman - 30/03/21 ---
       --left join web_sol_principal w on w.n_id_solicitud = ap.n_id_solicitud
       left join web_sol_principal w on w.n_id_solicitud = nvl(ap.n_id_solicitud, (SELECT max(ap3.n_id_solicitud)
                                                                                    ...
                                                                                    and ap3.c_identificacion = ap.c_identificacion --ale 31/03/21
                                                                                    )
                                                               )
...
and verificar_estado_alumno_rango(ap.n_id_alu_prog,'ACTIVO', :p1_fecha_desde,NVL(:p1_fecha_hasta,trunc(SYSDATE))) = 'TRUE'
and exists (select 1 
                from alumnos_programas ap2
                     join becas_alumnos_cab bac on bac.n_id_alu_prog = ap2.n_id_alu_prog
                                                and :p1_fecha_hasta between bac.f_desde and bac.f_hasta
